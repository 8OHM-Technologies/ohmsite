...
class DatasetManagementTest extends TestCase
{
    ...
    public function test_admin_can_create_dataset_with_demo_data(): void
    {
        $admin = User::factory()->create(['role' => 'admin']);
        ...
        $response = $this->actingAs($admin)->post(route('admin.datasets.store'), [
            'name' => 'JSON Dataset',
            'slug' => 'json-dataset',
            'description' => 'A dataset with JSON sample',
            'is_active' => 1,
            'demo_data' => json_encode([['key' => 'value']]),
        ]);
    ...
    public function test_admin_cannot_create_dataset_with_invalid_json_demo_data(): void
    {
        $admin = User::factory()->create(['role' => 'admin']);

        $response = $this->actingAs($admin)->post(route('admin.datasets.store'), [
            'name' => 'Invalid JSON Dataset',
            'slug' => 'invalid-json-dataset',
            'description' => 'A dataset with invalid JSON sample',
            'is_active' => 1,
            'demo_data' => '{invalid-json}',
        ]);

        $response->assertSessionHasErrors('demo_data');
    }

    public function test_public_can_access_dataset_sample(): void
    {
        $dataset = Dataset::create([
            'name' => 'Public Dataset',
            'slug' => 'public-dataset',
        ...
        $response = $this->get(route('datasets.sample', $dataset->slug));

        $response->assertStatus(200);
        $response->assertJson([['foo' => 'bar']]);
        $response->assertJsonMissing([['date_modified' => '2026-07-01']]);
        $response->assertJsonMissing([['detail_url' => 'https://...']]);
    }
}
