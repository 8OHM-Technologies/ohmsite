...
<template>
    <MainLayout :auth="auth" title="Shop">
        ...
        <div class="relative z-10">
            ...
            <div
                class="lg:hidden flex items-center justify-between mb-8 sticky top-24 z-30 bg-black/80 backdrop-blur-xl p-4 rounded-3xl border border-white/5">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative group">
                        <select v-model="activeFilters.sort"
                        ...
                        <ChevronDown
                            class="absolute right-3 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-500 pointer-events-none" />
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-8 xl:gap-16">
                ...
                <aside class="hidden lg:block w-72 flex-shrink-0 space-y-16 sticky top-40 h-fit">
                    <div>
                        <div class="flex items-end gap-3 mb-10">
                            <h3 class="text-4xl font-black uppercase tracking-tighter leading-none">Filters</h3>
                        ...
                        <div class="space-y-8 mb-16">
                            <div class="flex items-center justify-between">
                                <h4 class="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-700">Collection
                                </h4>
                            </div>
                            <div class="space-y-4">
                                <button @click="activeFilters.category = ''; applyFilters()"
                                    ...
                                    <span class="flex items-center gap-3">
                                        <div v-if="activeFilters.category === ''"
                                            class="w-1.5 h-1.5 bg-admin-modern rounded-full shadow-[0_0_10px_rgba(229,255,69,0.5)]">
                                        </div>
                                        All Styles
                                    </span>
                                ...
                                <button v-for="category in categories" :key="category.id"
                                    ...
                                    <span class="flex items-center gap-3">
                                        <div v-if="activeFilters.category === category.name"
                                            class="w-1.5 h-1.5 bg-admin-modern rounded-full shadow-[0_0_10px_rgba(229,255,69,0.5)]">
                                        </div>
                                        {{ category.name }}
                                    </span>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-6">
                                ...
                                <button @click="applyFilters"
                                    class="w-full bg-white text-black py-5 rounded-[2rem] font-black uppercase text-[10px] tracking-[0.3em] hover:bg-admin-modern transition-all shadow-2xl active:scale-95">
                                    Filter Results
                                </button>
                            </div>
                        </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="hidden lg:flex justify-between items-end mb-16">
                        ...
                        <div
                            class="flex items-center gap-8 bg-zinc-900/30 backdrop-blur-xl rounded-full py-3 px-8 border border-white/5">
                            <div class="flex items-center gap-3">
                                <span class="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-600">Sort
                                    By:</span>
                                <div class="relative group">
                                    <select v-model="activeFilters.sort"
                                    ...
                                    <ChevronDown
                                        class="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-500 pointer-events-none group-hover:text-white transition-colors" />
                                </div>
                            </div>
                            <div class="h-4 w-px bg-white/10"></div>
                            <div class="flex items-center gap-2">
                                <button class="p-2 text-white bg-white/5 rounded-full border border-white/10">
                                    <LayoutGrid class="w-3.5 h-3.5" />
                                </button>
                                <button class="p-2 text-zinc-600 hover:text-zinc-300 transition-colors">
                                    <Filter class="w-3.5 h-3.5" />
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- Main Grid -->
                    <div
                        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 sm:gap-8 xl:gap-10">
                        <div v-for="product in products.data" :key="product.id" class="group relative flex flex-col">
                            <!-- Favorite Button -->
                            <button @click.stop="toggleFavorite(product)" :class="[
                            ...
                            <Link :href="route('shop.show', product.id)" class="flex-1 flex flex-col">
                                <div
                                    ...
                                    <div v-if="product.sale_price && parseFloat(product.sale_price) < parseFloat(product.price)"
                                        class="absolute bottom-6 left-6 z-20 bg-admin-modern text-black px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl">
                                        OFFER
                                    </div>
                                </div>
                                ...
                                <div class="mt-6 px-4 flex-1 flex flex-col">
                                    <div class="flex justify-between items-start gap-4 mb-2">
                                        <h4
                                            class="font-black text-lg sm:text-xl text-white uppercase tracking-tighter leading-tight">
                                            {{ product.name }}</h4>
                                        <span
                    ...
                    <div v-if="products.links.length > 3" class="mt-24 flex justify-center flex-wrap gap-2">
                        <template v-for="(link, k) in products.links" :key="k">
                            <div v-if="link.url === null"
                                class="px-5 py-3 text-zinc-700 text-[10px] font-black uppercase opacity-50"
                    ...
                    <div v-if="products.data.length === 0" class="py-40 text-center flex flex-col items-center">
                        ...
                        <h3 class="text-4xl font-black uppercase tracking-tighter text-zinc-800 mb-4">No results found
                        </h3>
        ...
        <Transition enter-active-class="transition duration-500 ease-out" enter-from-class="translate-x-full"
            enter-to-class="translate-x-0" leave-active-class="transition duration-400 ease-in"
            ...
            <div v-if="isFilterDrawerOpen" class="fixed inset-0 z-[100] lg:hidden">
                ...
                <div
                    class="absolute inset-y-0 right-0 w-full sm:w-[450px] bg-zinc-900 shadow-2xl flex flex-col border-l border-white/5">
                    <div class="p-8 flex items-center justify-between border-b border-white/5">
                        <h3 class="text-2xl font-black uppercase tracking-tighter text-white">Refine Selection</h3>
                        <button @click="isFilterDrawerOpen = false" class="p-2 text-zinc-500 hover:text-white">
                            <X class="w-6 h-6" />
                        </button>
                    </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button
                                    ...
                                    :key="cat.name" @click="activeFilters.category = cat.val"
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    ...
                                    <input type="number" v-model="activeFilters.min_price"
                                        class="w-full bg-black border-white/5 rounded-xl py-4 px-4 text-xs font-bold text-white" />
                                </div>
                                <div class="space-y-2">
