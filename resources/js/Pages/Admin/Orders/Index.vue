...
<template>
    ...
    <AdminLayout>
        <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <button
                        class="bg-white/5 hover:bg-white/10 text-white px-6 py-3 rounded-2xl font-bold transition-all border border-white/5">
                        Export Orders
                    </button>
                </div>
            </div>
            ...
            <div
                ...
                <div class="relative flex-1 group w-full">
                    ...
                    <input v-model="search" type="text" placeholder="Search by Order ID or customer..."
                ...
                <div class="flex gap-4 w-full md:w-auto">
                    <select v-model="status"
                        class="bg-white/5 border-none rounded-2xl text-white py-3 px-6 focus:ring-2 focus:ring-admin-modern transition-all cursor-pointer min-w-[150px]">
            ...
            <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 rounded-[2.5rem] overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-white/5">
                                <th class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest">Order ID
                                </th>
                                <th class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest">Customer
                                </th>
                                <th class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest">Date
                                </th>
                                <th
                                    class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest text-right">
                                    Total</th>
                                <th class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest">Status
                                </th>
                                <th
                                    class="px-8 py-6 text-xs font-bold text-zinc-500 uppercase tracking-widest text-right">
                                    Actions</th>
                ...
                <div class="px-8 py-6 border-t border-white/5 flex items-center justify-between">
                    <p class="text-sm text-zinc-500">
                        Page <span class="text-white">{{ orders.current_page }}</span> of <span class="text-white">{{
        ...
        <div v-if="isDrawerOpen" class="fixed inset-0 z-50 overflow-hidden pointer-events-none">
            ...
            <div
                class="absolute inset-y-0 right-0 max-w-2xl w-full bg-zinc-950/95 backdrop-blur-2xl border-l border-white/10 shadow-2xl pointer-events-auto">
                <div class="h-full flex flex-col p-10 overflow-y-auto custom-scrollbar" v-if="selectedOrder">
                    <div class="flex items-center justify-between mb-10">
                        ...
                        <button @click="closeDrawer"
                            class="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white transition-all">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-4">
                                <div v-for="item in selectedOrder.items" :key="item.id"
                                    class="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 group">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-16 h-16 rounded-xl bg-zinc-800 overflow-hidden border border-white/10 group-hover:border-admin-modern/30 transition-all">
                                            <img :src="item.product?.image" class="w-full h-full object-cover" />
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-white">{{ item.product?.name }}</p>
                                            <p class="text-[10px] text-zinc-500 font-black uppercase">Qty: {{
                                                item.quantity }} × {{ formatCurrency(item.unit_price) }}</p>
                                        </div>
                                    </div>
                                    <p class="text-sm font-black text-white">{{ formatCurrency(item.quantity *
                                        item.unit_price) }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6 pt-6 border-t border-white/5">
                            ...
                            <div class="flex gap-4">
                                ...
                                <button @click="updateOrderStatus('Cancelled')"
                                    class="flex-1 py-4 px-6 border border-red-500/50 text-red-500 rounded-2xl font-black uppercase tracking-widest hover:bg-red-500/10 transition-all">
                                    Cancel Order
                                </button>
                            </div>
                        </div>
