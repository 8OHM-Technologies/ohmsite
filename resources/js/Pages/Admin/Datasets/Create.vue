    ...
    <AdminLayout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-10 gap-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h1 class="text-3xl sm:text-4xl font-black uppercase tracking-tighter text-white">Create New Dataset
                    </h1>
                    <p class="text-zinc-500 font-bold uppercase tracking-widest text-[10px]">Add a new legal case
                        library or intelligence dataset</p>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-zinc-900/40 rounded-[2.5rem] border border-white/5 p-6 sm:p-10">
                    ...
                    <div class="space-y-8">
                        ...
                        <div>
                            <InputLabel for="demo_data" value="Demo JSON Data"
                                class="text-zinc-600 font-black uppercase tracking-widest text-[10px] mb-3 ml-1" />
                            <textarea id="demo_data" v-model="form.demo_data" rows="12"
                                placeholder='[\n    {\n        "title": "Case 1",\n        "document_type": "CCMA Award",\n        ...\n    }\n]'
                                class="w-full bg-black/40 border border-white/5 text-zinc-300 rounded-xl p-6 focus:ring-1 focus:ring-admin-modern/40 transition-all font-mono text-xs leading-relaxed"
                                @input="validateJson"></textarea>
                            <div v-if="jsonError" class="text-rose-500 text-[10px] font-black uppercase tracking-wider mt-2 ml-1">
                                {{ jsonError }}
                            </div>
                            <InputError class="mt-2" :message="form.errors.demo_data" />
                        </div>
                    </div>
                </div>
            </div>
