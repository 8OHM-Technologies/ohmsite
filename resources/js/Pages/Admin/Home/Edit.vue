...
const form = useForm({
    ...
    section_headers: props.sectionHeaders?.value || defaultSectionHeaders,
});
        ...
        <div class="space-y-12 pb-20">
            ...
            <div class="bg-zinc-900/50 overflow-hidden shadow-sm sm:rounded-[2.5rem] p-10 border border-white/5">
                <div class="flex justify-between items-center mb-10">
                    <h3 class="text-2xl font-black text-white uppercase tracking-tighter">Hero Slider (Max 5)</h3>
                    <span
                        class="bg-zinc-800/50 text-zinc-500 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">{{
                            form.hero_slider.length }}/5 Slides</span>
                </div>

                <div class="space-y-8">
                    <div v-for="(slide, index) in form.hero_slider" :key="index"
                        class="relative p-10 border border-white/5 rounded-[2rem] bg-zinc-800/20 hover:bg-zinc-800/40 transition-all duration-500 group">
                        <button @click="removeHeroSlide(index)"
                            class="absolute top-8 right-8 text-zinc-500 hover:text-red-500 transition-all bg-zinc-900 rounded-xl p-2 shadow-sm border border-white/10">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label
                                        class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Select
                                        Product Source</label>
                                    <select v-model="slide.product_id"
                                ...
                                <div>
                                    <label
                                        class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Display
                                        Title</label>
                            ...
                            <div class="space-y-4">
                                <label
                                    class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Slide
                                    Media</label>
                                <div class="aspect-square rounded-[2.5rem] border-2 border-dashed border-white/5 flex flex-col items-center justify-center relative overflow-hidden group/img cursor-pointer hover:border-admin-modern transition-all bg-zinc-900"
                                    @click="$refs['fileInput' + index][0].click()">
                                    <img v-if="heroPreviews[index]" :src="heroPreviews[index]"
                                        class="h-48 object-contain transform -rotate-12 transition-transform duration-700 group-hover/img:scale-110 drop-shadow-2xl" />
                                    <div v-else class="text-center space-y-2">
                                        <svg class="w-10 h-10 text-zinc-700 mx-auto" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                d="M12 4v16m8-8H4" />
                                        </svg>
                                        <span
                                            class="text-[10px] font-black text-zinc-700 uppercase tracking-widest">Click
                                            to upload image</span>
                                    </div>

                                    <input :ref="'fileInput' + index" type="file" class="hidden" accept="image/*"
                                        @change="(e) => handleHeroImage(e, index)" />

                                    <div
                                        class="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                        <span
                                            class="text-white text-[10px] font-black uppercase tracking-[0.3em] border-2 border-white/20 px-6 py-2 rounded-full">Replace
                                            Media</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center">
                                <div
                                    class="bg-black rounded-[2rem] p-8 w-full text-zinc-500 space-y-4 shadow-2xl border border-white/5">
                                    <div class="flex items-center space-x-3 mb-2">
                                        <div
                                            class="w-2 h-2 bg-admin-modern rounded-full shadow-[0_0_8px_rgba(159,232,112,0.5)]">
                                        </div>
                                        <span
                                            class="text-[10px] font-black uppercase tracking-widest text-white">Premium
                                            Preview</span>
                                    </div>
                                    <p class="text-xs leading-relaxed font-medium">
                                        The title and price will be overlaid on the high-contrast background. Ensure
                                        your product image is transparent (PNG) for the best "floating" effect.
                                    </p>
                                    <div class="pt-4 flex items-center gap-2">
                                        <div class="h-1 flex-1 bg-zinc-800 rounded-full overflow-hidden">
                                            <div class="h-full bg-admin-modern w-[60%]"></div>
                                        </div>
                                        <span class="text-[10px] font-black text-zinc-400">HD Quality</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <button v-if="form.hero_slider.length < 5" @click="addHeroSlide" type="button"
            ...
            <!-- About Us Settings -->
            <div class="bg-zinc-900/50 overflow-hidden shadow-sm sm:rounded-[2.5rem] p-10 border border-white/5">
                <h3 class="text-2xl font-black text-white uppercase mb-10 tracking-tighter">About Us Narrative</h3>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    ...
                    <div class="space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Title
                                    Line 1 (Primary)</label>
                                <input v-model="form.about_us.title_line_1" type="text"
                            ...
                            <div>
                                <label
                                    class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Title
                                    Line 2 (Accent)</label>
                                <input v-model="form.about_us.title_line_2" type="text"
                                    class="w-full border-white/5 border rounded-2xl py-4 px-6 focus:ring-2 focus:ring-admin-modern focus:border-admin-modern transition-all bg-zinc-900 font-bold text-white" />
                            </div>
                        </div>
                        <div>
                            <label
                                class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Brand
                                Story / Description</label>
                            <textarea v-model="form.about_us.description" rows="5"
                        ...
                        <div class="grid grid-cols-2 gap-8 pt-4">
                            <div v-for="(stat, index) in form.about_us.stats" :key="index"
                                class="p-8 border border-white/5 rounded-[2rem] bg-zinc-800/20">
                                <label
                                    class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Metric
                                    Label</label>
                                <input v-model="stat.label" type="text"
                                    class="w-full border-0 border-b border-white/10 focus:border-admin-modern focus:ring-0 transition-all bg-transparent p-0 mb-4 font-bold text-sm text-white" />
                                <label
                                    class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Metric
                                    Value</label>
                                <input v-model="stat.value" type="text"
                                    class="w-full border-0 border-b border-white/10 focus:border-admin-modern focus:ring-0 transition-all bg-transparent p-0 font-black text-3xl tracking-tighter text-admin-modern" />
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Right: Image -->
                    <div class="space-y-6">
                        <label class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Section
                            Visual</label>
                        <div
                            class="relative aspect-[4/5] rounded-[3rem] overflow-hidden border border-white/5 shadow-2xl group">
                            <img v-if="aboutUsPreview" :src="aboutUsPreview"
                                class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                            <div v-else
                                class="w-full h-full bg-zinc-800 flex items-center justify-center text-zinc-600 font-black uppercase tracking-widest">
                                No Visual Loaded</div>

                            <label
                                class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer text-white backdrop-blur-sm">
                                ...
                                <span
                                    class="font-black uppercase text-xs tracking-[0.3em] border-2 border-white/20 px-8 py-3 rounded-full hover:bg-admin-modern hover:text-black hover:border-admin-modern transition-colors">Update
                                    Story Visual</span>
            ...
            <div class="bg-zinc-900/50 overflow-hidden shadow-sm sm:rounded-[2.5rem] p-10 border border-white/5">
                ...
                <div class="space-y-6">
                    <div v-for="(item, index) in form.roadmap" :key="index"
                        class="relative p-8 border border-white/5 rounded-[2rem] bg-zinc-800/20 hover:bg-zinc-800/40 transition-all duration-500 group">

                        <!-- Controls -->
                        <div class="absolute top-6 right-6 flex items-center gap-2">
                            <button @click="moveRoadmapItem(index, -1)" :disabled="index === 0"
                            ...
                            <button @click="removeRoadmapItem(index)"
                                class="text-zinc-500 hover:text-red-500 transition-all bg-zinc-900 rounded-xl p-2 shadow-sm border border-white/10">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                        ...
                        <div class="mb-6">
                            <span
                                class="bg-zinc-800 text-zinc-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Item
                                {{ index + 1 }}</span>
                        </div>
            ...
            <div class="bg-zinc-900/50 overflow-hidden shadow-sm sm:rounded-[2.5rem] p-10 border border-white/5">
                ...
                <div class="space-y-8">
                    <div v-for="(section, key) in form.section_headers" :key="key"
                        class="p-8 border border-white/5 rounded-[2rem] bg-zinc-800/20 hover:bg-zinc-800/40 transition-all duration-500">
