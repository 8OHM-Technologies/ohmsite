        ...
        <div class="p-10 space-y-10">
            ...
            <div class="flex gap-4">
                <input v-model="search" placeholder="Search customers..."
                    class="bg-zinc-900/50 border border-white/5 rounded-2xl py-3 px-6 text-white w-full md:w-80 focus:ring-2 focus:ring-admin-modern transition-all" />
                <select v-model="status"
                    class="bg-zinc-900/50 border border-white/5 rounded-2xl py-3 px-6 text-white focus:ring-2 focus:ring-admin-modern transition-all">
            ...
            <!-- Table -->
            <div v-if="customers && customers.data"
                class="bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
                        ...
                        <tr v-for="customer in customers.data" :key="customer.id"
                            ...
                            <td class="p-6 text-right">
                                <div class="flex justify-end gap-3">
                                    <Link v-if="customer.id" :href="route('admin.customers.show', customer.id)"
                                        class="p-2 bg-white/5 hover:bg-admin-modern hover:text-black rounded-xl transition-all">
                                    ...
                                    <button v-if="customer.id"
                                        ...
                                        <svg v-if="customer.is_banned" class="w-5 h-5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div v-if="customers && customers.links && customers.links.length > 3" class="flex justify-center gap-2">
                <template v-for="(link, index) in customers.links" :key="index">
                    <Link v-if="link.url" :href="link.url" v-html="link.label"
                        class="px-5 py-3 rounded-2xl text-[10px] font-black uppercase transition-all" :class="[
                    ...
                    <span v-else v-html="link.label"
                        class="px-5 py-3 rounded-2xl text-[10px] font-black uppercase bg-zinc-900/50 border border-white/5 text-zinc-700 cursor-not-allowed"></span>
                </template>
            </div>
        </div>
    </AdminLayout>
