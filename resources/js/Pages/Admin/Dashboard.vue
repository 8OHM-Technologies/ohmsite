...
<template>
    ...
    <AdminLayout>
        ...
        <div
            ...
            <img src="/assets/images/hero-bg-standard.png" alt="New Drop"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 mb-8 lg:mb-12">
            <div v-for="stat in stats" :key="stat.name"
                class="bg-zinc-900/40 p-6 lg:p-8 rounded-[2rem] border border-white/5 group hover:bg-zinc-900/60 transition-all duration-300">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="p-3.5 bg-zinc-800/80 border border-white/5 rounded-2xl group-hover:bg-admin-modern transition-all duration-300 shadow-xl group-hover:shadow-admin-modern/10">
                        <svg class="h-5 w-5 text-white group-hover:text-black transition-colors" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :d="stat.icon" />
                        </svg>
                    </div>
                    <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest"
                ...
                <div>
                    ...
                    <h3 class="text-3xl lg:text-4xl font-black text-white uppercase tracking-tighter">{{ stat.value }}
                    </h3>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8 xl:items-start">
            ...
            <div class="xl:col-span-2 bg-zinc-900/40 rounded-[2.5rem] p-6 sm:p-10 border border-white/5 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 lg:mb-12 gap-4">
                    ...
                    <Link :href="route('admin.products.index')"
                        class="inline-flex items-center text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-all group">
                        Browse all
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                    <div v-for="product in recentProducts" :key="product.id"
                        class="group relative bg-black/20 rounded-[2rem] p-6 hover:bg-zinc-800/40 transition-all duration-500 border border-white/5">
                        <div
                            class="aspect-square rounded-3xl bg-zinc-900/50 overflow-hidden mb-6 flex items-center justify-center p-8 border border-white/5">
                            <img :src="product.image" :alt="product.name"
                                class="w-full h-full object-contain group-hover:scale-110 group-hover:-rotate-6 transition-all duration-1000 drop-shadow-[0_20px_20px_rgba(0,0,0,0.4)]" />
                        </div>
                        <div class="flex justify-between items-end">
                            <div class="flex-1 mr-4">
                                <p class="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1">In Stock:
                                    {{ product.stock }}</p>
            ...
            <div
                class="bg-black rounded-[2.5rem] p-6 sm:p-10 border border-white/5 shadow-2xl relative overflow-hidden group">
                <div
                    ...
                    <div class="space-y-6">
                        <div class="h-64 flex items-end justify-between gap-1 sm:gap-2 px-2">
                            <div v-for="sale in weeklySales" :key="sale.day" class="flex-1 group relative">
                                ...
                                <span
                                    class="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] font-black text-zinc-600 uppercase group-hover:text-white transition-colors">{{
                                        sale.day }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-10 border-t border-white/5">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex flex-col">
                                <p class="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em] mb-1">
                                    Performance Index</p>
                                <span class="text-white font-black text-3xl tracking-tighter">{{ growthRate }}</span>
                            </div>
                        ...
                        <div class="w-full h-1.5 bg-zinc-900 rounded-full overflow-hidden">
                            <div class="h-full rounded-full shadow-[0_0_20px_rgba(159,232,112,0.4)] transition-all duration-1000"
                                :class="[parseFloat(growthRate) >= 0 ? 'bg-admin-modern' : 'bg-rose-500']"
                                :style="{ width: Math.min(Math.abs(parseFloat(growthRate)), 100) + '%' }"></div>
                        </div>
                    </div>
