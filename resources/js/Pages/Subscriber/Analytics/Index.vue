...
<template>
    ...
    <SubscriberLayout>
        <div class="space-y-8 animate-in fade-in duration-700">
            ...
            <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-6 rounded-[2rem] space-y-4">
                ...
                <div v-if="['courts', 'gazettes_journals', 'other'].includes(activeGroup)"
                    ...
                    <select :value="selectedDataset" @change="selectDataset($event.target.value)"
                        ...
                        <option v-for="f in getFiltersForGroup(activeGroup)" :key="f.target_name"
                            :value="f.target_name">
                            {{ f.vanity_name }}
                        </option>
                    </select>
            ...
            <template v-else-if="datasetMode === 'ccma' && analyticsData">
                ...
                <div class="bg-zinc-900/30 backdrop-blur-md border border-white/5 p-6 rounded-[2rem] space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <h4 class="text-xs font-black uppercase tracking-widest text-zinc-300">Active Analytics
                                Filters</h4>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="flex flex-col space-y-1.5">
                            ...
                            <select v-model="filterCategory"
                                ...
                                <option
                                    v-for="c in ['Misconduct', 'Retrenchment', 'Incapacity', 'Unfair Labor Practice', 'Constructive Dismissal', 'Unfair Dismissal', 'Mutual Interest', 'Other']"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div
                        ...
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-[9px] font-black text-admin-modern bg-admin-modern/10 px-2 py-1 rounded-lg">LIVE</span>
                        </div>
                    ...
                    <div
                        ...
                        <p class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Avg Hearing Duration
                        </p>
                        <p class="text-3xl font-black text-white mt-1 tracking-tighter">{{ avgHearingDuration }} Day(s)
                        </p>
                    </div>
                    <div
                        class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] group hover:bg-zinc-900/60 transition-all duration-300">
                ...
                <div v-if="activeTab === 'overview'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Monthly Labor
                            Dispute Trends</h3>
                        <VueApexCharts type="area" height="260" :options="monthlyTrendOptions"
                            :series="monthlyTrendSeries" />
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Dispute Category Mix
                        </h3>
                        <VueApexCharts type="donut" height="260" :options="disputeTypeChartOptions"
                            :series="disputeTypeSeries" />
                    </div>
                    <div class="lg:col-span-3 bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Recent Award
                            Activity</h3>
                        <div class="space-y-3">
                            <div v-for="c in recentCasesStream" :key="c.id"
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-zinc-500 mt-0.5">{{ c.employer }} · {{ c.court_location
                                    }}</p>
                                </div>
                                <span
                                    class="text-[10px] font-black px-2 py-1 rounded-lg bg-admin-modern/10 text-admin-modern shrink-0 ml-4">{{
                                        c.category }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else-if="activeTab === 'velocity'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Procedural Velocity
                            by Region</h3>
                        <VueApexCharts type="bar" height="300" :options="velocityChartOptions"
                            :series="velocitySeries" />
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] space-y-3">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest">Top 5 Regions by Case
                            Load</h3>
                        <div v-for="(region, idx) in velocityByRegionData.categories" :key="region"
                            ...
                            <div class="flex items-center gap-3">
                                <span
                                    class="w-6 h-6 rounded-lg bg-white/5 text-[10px] font-black flex items-center justify-center text-zinc-400">{{
                                        idx + 1 }}</span>
                                <span class="text-xs font-bold text-zinc-200">{{ region }}</span>
                            </div>
                            <span class="text-[10px] font-black text-admin-modern">{{
                                velocityByRegionData.hearingAvg[idx] }}d avg</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="activeTab === 'trends'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Provincial Case
                            Density</h3>
                        <VueApexCharts type="bar" height="300" :options="provincialDensityOptions"
                            :series="provincialDensitySeries" />
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Industry
                            Benchmarking</h3>
                        <div class="space-y-3 max-h-[300px] overflow-y-auto">
                            <div v-for="ind in industryBenchmarking" :key="ind.name"
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-zinc-500">{{ ind.count }} cases · {{ ind.share }}% share
                                    </p>
                                </div>
                                <span class="text-[10px] font-black text-zinc-400">{{ ind.avgHearing }}d / {{
                                    ind.avgAward }}d</span>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else-if="activeTab === 'employer-risk'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Top Repeat Litigants
                        </h3>
                        <div class="space-y-2 max-h-[340px] overflow-y-auto">
                            <div v-for="(emp, idx) in repeatAppellants" :key="emp.name"
                                ...
                                <span
                                    class="text-[10px] font-black px-2 py-1 bg-admin-modern/10 text-admin-modern rounded-lg">{{
                                        emp.count }} cases</span>
                            </div>
                        </div>
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] space-y-4">
                        <div class="flex items-center gap-3">
                            <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest">Employer Profiler
                            </h3>
                        </div>
                        ...
                        <template v-if="profileEmployerStats">
                            ...
                            <VueApexCharts type="donut" height="220" :options="employerSignatureChartOptions"
                                :series="employerSignatureSeries" />
                        </template>
                        <div v-else class="py-10 text-center text-zinc-500 text-xs font-bold uppercase tracking-widest">
                            Select an employer above
                        </div>
                    </div>
                </div>
            ...
            <template v-else-if="datasetMode === 'legal' && analyticsData">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div
                        class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] group hover:bg-zinc-900/60 transition-all">
                        <div class="flex items-center justify-between mb-6">
                            <div
                                class="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-zinc-400 group-hover:text-admin-modern transition-colors">
                                <FileText class="w-5 h-5" />
                            ...
                            <span
                                class="text-[9px] font-black text-admin-modern bg-admin-modern/10 px-2 py-1 rounded-lg">TOTAL</span>
                        </div>
                        ...
                        <p class="text-3xl font-black text-white mt-1 tracking-tighter">{{
                            legalTotals.total.toLocaleString() }}</p>
                    </div>
                    <div
                        class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] group hover:bg-zinc-900/60 transition-all">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-[9px] font-black text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-lg">INDEXED</span>
                        </div>
                        <p class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">With Case Number</p>
                        <p class="text-3xl font-black text-white mt-1 tracking-tighter">{{
                            legalTotals.with_case_number.toLocaleString() }}</p>
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-[9px] font-black text-sky-400 bg-sky-400/10 px-2 py-1 rounded-lg">DATED</span>
                        </div>
                        <p class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Records with Date</p>
                        <p class="text-3xl font-black text-white mt-1 tracking-tighter">{{
                            legalTotals.with_date.toLocaleString() }}</p>
                        <p class="text-[10px] text-zinc-500 mt-1" v-if="legalTotals.total > 0">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-[9px] font-black text-purple-400 bg-purple-400/10 px-2 py-1 rounded-lg">TYPES</span>
                        </div>
                        <p class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Document Types</p>
                        <p class="text-3xl font-black text-white mt-1 tracking-tighter">{{
                            Object.keys(legalByDocumentType).length }}</p>
                    </div>
                </div>
                ...
                <div v-if="activeTab === 'overview'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        ...
                        <VueApexCharts v-if="Object.keys(legalByYear).length" type="area" height="260"
                            :options="legalYearChartOptions" :series="legalYearSeries" />
                        <div v-else
                            class="flex items-center justify-center h-[260px] text-zinc-600 text-xs font-bold uppercase tracking-widest">
                            No dated records available</div>
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Document Type Mix
                        </h3>
                        <VueApexCharts v-if="legalDocTypeSeries.length" type="donut" height="260"
                            :options="legalDocTypeChartOptions" :series="legalDocTypeSeries" />
                        <div v-else
                            class="flex items-center justify-center h-[260px] text-zinc-600 text-xs font-bold uppercase tracking-widest">
                            No type data</div>
                    </div>
                ...
                <div v-else-if="activeTab === 'temporal'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Annual Publication
                            Volume</h3>
                        <VueApexCharts v-if="Object.keys(legalByYear).length" type="area" height="300"
                            :options="legalYearChartOptions" :series="legalYearSeries" />
                        <div v-else
                            class="flex items-center justify-center h-[300px] text-zinc-600 text-xs uppercase tracking-widest">
                            No data</div>
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Monthly Distribution
                        </h3>
                        <VueApexCharts v-if="Object.keys(legalByMonth).length" type="bar" height="300"
                            :options="legalMonthChartOptions" :series="legalMonthSeries" />
                        <div v-else
                            class="flex items-center justify-center h-[300px] text-zinc-600 text-xs uppercase tracking-widest">
                            No data</div>
                    </div>
                ...
                <div v-else-if="activeTab === 'courts'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-6">Top Courts by Record
                            Volume</h3>
                        <VueApexCharts v-if="legalTopCourts.length" type="bar" height="340"
                            :options="legalCourtsChartOptions" :series="legalCourtsSeries" />
                        <div v-else
                            class="flex items-center justify-center h-[340px] text-zinc-600 text-xs uppercase tracking-widest">
                            No court data</div>
                    </div>
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        ...
                        <div class="space-y-2 max-h-[340px] overflow-y-auto">
                            <div v-for="(court, idx) in legalTopCourts" :key="court.court"
                                ...
                                <span
                                    class="text-[10px] font-black px-2 py-1 bg-purple-500/10 text-purple-300 rounded-lg">{{
                                        court.count.toLocaleString() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="activeTab === 'documents'" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem]">
                        <h3 class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">Document Type
                            Breakdown</h3>
                        <VueApexCharts v-if="legalDocTypeSeries.length" type="donut" height="280"
                            :options="legalDocTypeChartOptions" :series="legalDocTypeSeries" />
                        <div class="space-y-2 mt-4">
                            <div v-for="(count, type) in legalByDocumentType" :key="type"
                                class="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                                <span class="text-xs font-black text-zinc-200 capitalize">{{ type }}</span>
                                <span class="text-[10px] font-black text-admin-modern">{{ count.toLocaleString()
                                }}</span>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-2 mt-1 flex-wrap">
                                            <span v-if="record.case_number"
                                                class="text-[10px] font-mono text-zinc-500">{{ record.case_number
                                                }}</span>
                                            <span v-if="record.court"
                                                class="text-[10px] font-black px-1.5 py-0.5 bg-purple-500/10 text-purple-300 rounded">{{
                                                    record.court }}</span>
                                            <span v-if="record.document_date" class="text-[10px] text-zinc-500">{{
                                                record.document_date }}</span>
                                        </div>
                                    </div>
            ...
            <div v-else-if="!analyticsLoading"
                ...
                <p class="text-zinc-500 font-black uppercase tracking-widest text-xs">No analytics data available for
                    this dataset
                    yet.</p>
        ...
        <div v-if="isMetricsModalOpen"
            ...
            @click.self="isMetricsModalOpen = false">
            <div class="bg-zinc-950 border border-white/10 rounded-[2rem] p-8 max-w-lg w-full space-y-4 shadow-2xl">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="isMetricsModalOpen = false" class="text-zinc-500 hover:text-white transition">
                        <X class="w-5 h-5" />
                    </button>
                </div>
                <div class="space-y-3 text-xs text-zinc-300">
                    <div class="border-b border-white/5 pb-3"><strong class="text-white">Hearing Duration:</strong>
                        (hearing_end
                        - hearing_start) + 1 day</div>
                    <div class="border-b border-white/5 pb-3"><strong class="text-white">Time-to-Award:</strong>
                        award_date -
                        hearing_end (calendar days)</div>
                    <div class="border-b border-white/5 pb-3"><strong class="text-white">Data Latency:</strong>
                        details_scraped_at - award_date (days)</div>
                    <div><strong class="text-white">Dispute Category:</strong> Parsed from reason_for_dismissal field
                        via
                        keyword matching</div>
                </div>
            </div>
        </div>
