...
<template>
    <div
        class="flex py-6 space-x-4 border-b border-gray-200 dark:border-gray-800 last:border-0 group animate-in fade-in slide-in-from-bottom-2 duration-300">
        <!-- Image -->
        <div
            class="relative flex-shrink-0 w-24 h-24 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800 sm:w-32 sm:h-32">
            <img :src="item.product.image || '/assets/images/placeholder.png'" :alt="item.product.name"
                ...
                loading="lazy" />
            <div v-if="hasSale"
                class="absolute top-2 left-2 px-2 py-1 text-[10px] font-bold text-white uppercase bg-rose-500 rounded">
                Sale
            </div>
        </div>
        ...
        <div class="flex flex-col flex-1">
            <div class="flex justify-between text-base font-medium text-gray-900 dark:text-white">
                <div>
                    <h3 class="hover:text-primary transition-colors">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 font-bold uppercase tracking-tight">
                        ...
                        <span v-if="item.options?.frequency">Billing: {{ getFrequencyLabel(item.options.frequency)
                        }}</span>
                ...
                <div class="text-right">
                    <p v-if="hasSale" class="text-rose-500 font-bold">
                        ...
                        <span class="block text-xs text-gray-400 line-through">{{ formattedPrice(item.product.price)
                        }}</span>
                    </p>
            ...
            <div class="flex items-center justify-between flex-1 mt-4">
                <!-- Quantity Stepper -->
                <div
                    class="flex items-center border border-gray-200 dark:border-gray-700 rounded-full bg-white dark:bg-gray-900 overflow-hidden">
                    <button @click="handleQuantityChange(item.quantity - 1)" :disabled="item.quantity <= 1"
                    ...
                    <button @click="handleQuantityChange(item.quantity + 1)"
                ...
                <div class="flex items-center space-x-4">
                    <button @click="emit('toggle-favorite')"
                        class="p-2 text-gray-400 hover:text-rose-500 transition-colors" title="Save for later">
                        <Heart class="w-5 h-5" />
                    </button>
                    <button @click="emit('remove')" class="p-2 text-gray-400 hover:text-rose-500 transition-colors"
                        title="Remove item">
