...
<template>
    <div v-if="items.length > 0" class="mt-16 animate-in fade-in duration-700">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-xl font-bold text-primary">Saved for later</h2>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="item in items" :key="item.id"
                ...
                <p class="text-sm text-indigo-600 dark:text-indigo-400 font-bold mb-4">{{ formattedPrice(item.price) }}
                </p>

                <button @click="emit('move-to-cart', item.id)"
